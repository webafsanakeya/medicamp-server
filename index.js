...
// Use a single env var name for the JWT secret
const JWT_SECRET = process.env.JWT_SECRET || process.env.ACCESS_TOKEN_SECRET;
if (!JWT_SECRET) {
  console.warn("Warning: JWT_SECRET is not set in env. Tokens will fail.");
}
...
async function run() {
  try {
    // connect BEFORE using collections
    ...
    // Example: header-based JWT verification (Authorization: Bearer <token>)
    ...
    // // Cookie-based token verification (if you prefer cookies)
    // const verifyTokenFromCookie = (req, res, next) => {
    //   const token = req.cookies?.token;
    //   if (!token) return res.status(401).send({ message: "unauthorized access" });
    //   jwt.verify(token, JWT_SECRET, (err, decoded) => {
    //     if (err) return res.status(401).send({ message: "unauthorized access" });
    //     req.user = decoded; // for legacy usage
    //     next();
    //   });
    // };
 ...
 // JWT Token API
    ...
    // // Example cookie-based JWT (if you want to set cookie instead)
    ...
    console.log("MongoDB connected and routes registered");
  } catch (err) {
    console.error("Mongo connection failed or server init error:", err);
    process.exit(1); // Crash early if DB connectivity is essential
  }
}
...
// // start local server only when not production (Vercel expects exported app)
